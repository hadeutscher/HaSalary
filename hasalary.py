...
def main():
    ...
    if independent_mode:
        ...
        pens_b = (
            PENSION_INDEPENDENT_RATE_REIMBURSE + PENSION_INDEPENDENT_RATE_REIMBURSE_ACA
        ) * salary_for_pens
        pens = pens_a + pens_b
        sfund = STUDY_FUND_INDEPENDENT * min(
            social_salary, STUDY_FUND_INDEPENDENT_MAX_SALARY
        )
        ...
        if pens_employer > PENSION_EMPLOYER_TAX_EXEMPT_PAYMENTS_MAX:
            # Zkifat Tagmulim
            tax_worth_features += (
                pens_employer - PENSION_EMPLOYER_TAX_EXEMPT_PAYMENTS_MAX
            )
        reparations = PENSION_REPARATIONS * social_salary
        if reparations > PENSION_REPARATIONS_TAX_EXEMPT_PAYMENTS_MAX:
            # Zkifat Pitzuiim
            tax_worth_features += (
                reparations - PENSION_REPARATIONS_TAX_EXEMPT_PAYMENTS_MAX
            )
    ...
    if independent_mode:
        ...
        if include_pension:
            ...
            tax_rate = (
                income_tax(monthly_reparations_pull, tax_pts) / monthly_reparations_pull
            )
            reparations_cash += taxed_reparations * (1 - tax_rate)
        employer_sfund = STUDY_FUND_EMPLOYER * (
            social_salary
            if full_study_fund
            else min(social_salary, STUDY_FUND_TAX_EXEMPT_MAX)
        )
    ...
    if target is not None:
        ...
        for i in itertools.count():
            if (
                current_cash * monthly_gain_rate**i
                ...
                break
        print(
            f"{years:.1f} years to reach target of {round(target)} with {round(monthly_gain)} monthly saving"
        )
